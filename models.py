...
from sqlalchemy.dialects.mysql import (
    TINYTEXT, CHAR, VARCHAR, TEXT, TIMESTAMP,
...
class Article(Base):
    ...
    aid = Column(
        INTEGER(unsigned=True, zerofill=True),
        autoincrement=True,
        primary_key=True)
...
class Comment(Base):
    ...
    cid = Column(
        INTEGER(unsigned=True, zerofill=True),
        primary_key=True)  # 此评论id
    aid = Column(
        INTEGER(unsigned=True, zerofill=True),
        nullable=False)  # 文章id
    pid = Column(
        INTEGER(unsigned=True, zerofill=True)
    )  # 父级评论的id，如果为空，则等于aid，即此评论是顶级评论
...
class Classes(Base):
    ...
    cls = Column(
        VARCHAR(15), nullable=False, unique=True
    )  # 最多15个字 | 自定义的分类
